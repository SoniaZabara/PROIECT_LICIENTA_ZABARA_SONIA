...
(This file is to test the interpretation of expressions.)
(It tests all unary and binary functions implemented.)
(It also tests parameter setting and referencing.)
(The last few lines test more complicated expressions. This program is not intended to be run on a machining center.)
(The tool path is just random back and forth on the X-axis.)
